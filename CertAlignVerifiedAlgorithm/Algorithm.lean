...
theorem align_optimal (S : Scoring) (x y : DNA) :
  ...
  · constructor
    · exact ⟨ed S x y, align_cost S x y⟩
    · intro q hq
      obtain ⟨cq, hqcost⟩ := validPath_cost_defined S x y q hq
      exact ⟨ed S x y, cq, align_cost S x y, hqcost, ed_lower_bound S q x y cq hqcost⟩
